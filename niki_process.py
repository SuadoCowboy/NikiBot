...
async def parseDiscordCommands(interaction: discord.Interaction, discordCommands: list[str], isUserIdInOwners: bool):
	for command in discordCommands:
		...
		while i < len(args):
			...
			if addArgs:
				args[argToAddIndex] += ' '+arg

				if arg.endswith('"'):
					args[argToAddIndex] = args[argToAddIndex][:-1]
					addArgs = False

				if len(args)-1 == i:
					args = args[:argToAddIndex+1]
					break
		...
		if command in commands.commands:
			await commands.commands[command](interaction, args, isUserIdInOwners)
